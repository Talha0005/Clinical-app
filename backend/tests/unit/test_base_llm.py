...
from llm.base_llm import ChatMessage, ConversationHistory, BaseLLM, MockLLM, LLMFactory
...
class TestChatMessage:
    ...
    def test_chat_message_to_dict(self):
        ...
        timestamp = datetime.now()
        message = ChatMessage(
            role="assistant", content="Hello, how can I help?", timestamp=timestamp
        )
...
class TestMockLLM:
    ...
    async def test_generate_response(self, mock_llm, test_conversation):
        ...
        response = await mock_llm.generate_response(
            test_conversation, "I have a headache"
        )
    ...
    async def test_generate_streaming_response(self, mock_llm, test_conversation):
        ...
        async for chunk in mock_llm.generate_streaming_response(
            test_conversation, "I feel dizzy"
        ):
            chunks.append(chunk)
    ...
    def test_register_new_provider(self):
        ...
        class TestLLM(BaseLLM):
            ...
            async def generate_streaming_response(
                self, conversation, new_message, **kwargs
            ):
...
async def test_conversation_concurrent_access():
    ...
    conversation = ConversationHistory(
        messages=[], conversation_id="concurrent-test", created_at=datetime.now()
    )
    ...
    await asyncio.gather(
        add_messages("task1", 5), add_messages("task2", 5), add_messages("task3", 5)
    )
