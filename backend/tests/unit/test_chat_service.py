...
class TestDigiClinicChatService:
    ...
    async def test_start_conversation_with_id(self, chat_service):
        ...
        conversation = await chat_service.start_conversation(conversation_id=conversation_id)
    ...
    async def test_start_conversation_existing(self, chat_service):
        ...
        conversation1 = await chat_service.start_conversation(conversation_id="existing-123")
        initial_message_count = len(conversation1.messages)
        
        # Retrieve same conversation
        conversation2 = await chat_service.start_conversation(conversation_id="existing-123")
        
        assert conversation1 is conversation2
        assert len(conversation2.messages) == initial_message_count  # No new messages added
    ...
    async def test_send_message_success(self, chat_service, sample_conversation_data):
        ...
        result = await chat_service.send_message(
            ...
            "Tell me more about my headache"
        )
    ...
    @pytest.mark.asyncio
    async def test_send_message_with_llm_error(self, chat_service, sample_conversation_data):
        """Test message sending when LLM fails."""
        # Mock LLM to raise exception
        with patch.object(chat_service.llm, 'generate_response', side_effect=Exception("LLM failed")):
            result = await chat_service.send_message(
                sample_conversation_data,
                "Test message"
            )
            ...
            assert result["conversation"] == sample_conversation_data  # Original data returned
    ...
    def test_create_conversation(self, chat_service):
        ...
        assert len(conversation.messages) == 0  # create_conversation doesn't add messages
    ...
    async def test_health_check_exception(self, chat_service):
        ...
        with patch.object(chat_service.llm, 'health_check', side_effect=Exception("Health check failed")):
            health = await chat_service.health_check()
