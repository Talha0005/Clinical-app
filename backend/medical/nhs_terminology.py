...
class NHSTerminologyServer(MedicalDataSource):
    ...
    def __init__(self, environment: str = "production1"):
        ...
        self.fhir_base_url = self.FHIR_ENDPOINTS.get(environment, self.FHIR_ENDPOINTS["production1"])
    ...
    async def _get_access_token(self) -> str:
        ...
        try:
            async with self.session.post(self.AUTH_URL, data=data) as response:
                ...
                logger.info(f"NHS Terminology Server token obtained, expires in {expires_in}s")
                return self.access_token
    ...
    async def _make_fhir_request(self, endpoint: str, params: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    async def search_snomed(self, text: str, limit: int = 20) -> List[TerminologyConcept]:
    ...
    async def search_medications(self, text: str, limit: int = 20) -> List[TerminologyConcept]:
        ...
        for url_pattern in url_patterns:
            try:
                ...
                response = await self._make_fhir_request("ValueSet/$expand", request_params)
                ...
                if medications:  # If we found results, return them
                    logger.info(f"dm+d search successful with URL pattern: {url_pattern}")
                    return medications
    ...
    async def search_icd10(self, text: str, limit: int = 20) -> List[TerminologyConcept]:
        ...
        for url_pattern in url_patterns:
            try:
                ...
                if concepts:  # If we found results, return them
                    logger.info(f"ICD-10 search successful with URL pattern: {url_pattern}")
                    return concepts
    ...
    async def search_opcs4(self, text: str, limit: int = 20) -> List[TerminologyConcept]:
        ...
        for url_pattern in url_patterns:
            try:
                ...
                if procedures:  # If we found results, return them
                    logger.info(f"OPCS-4 search successful with URL pattern: {url_pattern}")
                    return procedures
    ...
    async def expand_valueset(self, url: str, filter: Optional[str] = None) -> Optional[ValueSet]:
    ...
    async def text_to_code(self, text: str, system: str = "snomed", limit: int = 10) -> List[TerminologyConcept]:
        ...
        if system == "snomed":
            ...
            raise ValueError(f"Unsupported terminology system: {system}. Supported: snomed, dmd, icd10, opcs4")
    ...
    async def validate_code(self, code: str, system: str) -> bool:
        ...
        try:
            response = await self._make_fhir_request("CodeSystem/$validate-code", params)
    ...
    async def get_conditions_by_symptoms(self, symptoms: List[str], limit: int = 10) -> SearchResult:
