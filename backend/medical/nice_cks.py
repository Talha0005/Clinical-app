...
class NiceCksDataSource(MedicalDataSource):
    ...
    async def _get_mock_search_results(self, query: str, limit: int) -> List[MedicalCondition]:
    ...
    async def _get_mock_condition(self, name: str) -> Optional[MedicalCondition]:
        ...
        if "hypertension" in name.lower():
            return MedicalCondition(
                ...
                causes=["Lifestyle factors", "Genetics", "Underlying conditions", "Stress"],
                treatments=["ACE inhibitors", "Lifestyle changes", "Beta blockers", "Diuretics"],
                severity="Moderate",
                category="Cardiovascular",
                source_url=f"{self.base_url}/hypertension",
                last_updated="2024-01-15"
    ...
    async def _get_mock_symptom_results(self, symptoms: List[str], limit: int) -> List[MedicalCondition]:
