...
class DigiClinicLLMRouter:
    ...
    async def route_request(
        ...
        **kwargs,
        ...
        try:
            if not self.router:
                logger.info("LLM router not initialized; returning safe fallback")
    ...
    async def _handle_vision_request(
        self, messages: List[Dict], agent_type: AgentType, **kwargs
    ) -> str:
        ...
        try:
            ...
            logger.info(
                f"Vision request processed successfully: {len(response)} characters"
            )
            return response
        ...
        try:
            ...
            async def _try_model(target_model: str):
                return await acompletion(
                    model=target_model, messages=messages, router=self.router, **kwargs
                )
            ...
            try:
                ...
                logger.info(
                    "LLM response generated: %s -> %s", agent_type.value, model_name
                )
                return result
                ...
                should_fallback = (
                    "insufficient_quota" in err_txt
                    or "rate limit" in err_txt
                    or "429" in err_txt
                )
                ...
                fallback_models = [
                    m
                    for m in self.agent_model_mapping.get(agent_type, [])
                    if m != model_name
                ]
                ...
                for alt in fallback_models:
                    try:
                        ...
                        logger.info(
                            "Fallback succeeded: %s -> %s", agent_type.value, alt
                        )
                        return result
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                ...
                err_txt = str(e_primary).lower()
                logger.warning(
                    "Streaming start failed for %s: %s", chosen_model, e_primary
                )
                should_fallback = (
                    "insufficient_quota" in err_txt
                    or "rate limit" in err_txt
                ...
                if should_fallback and chosen_model.startswith("gpt"):
                    ...
                    for alt in ordered:
                        try:
                            ...
                            logger.warning(
                                "Streaming fallback failed for %s: %s", alt, e_alt
                            )
            ...
            logger.info(
                "Streaming response completed: %s -> %s", agent_type.value, chosen_model
            )
