...
class DigiClinicChatService:
    ...
    def __init__(self, llm_provider: str = "router", **llm_config):
        ...
        if llm_provider == "router":
            try:
                ...
                self.logger.info(
                    "Chat service initialized with LLM router and fallback LLM"
                )
            except Exception as e:
                self.logger = logging.getLogger(__name__)
                self.logger.error(
                    f"LLM router initialization failed, falling back to legacy LLM: {e}"
                )
                self.llm = LLMFactory.create_llm("mock", **llm_config)
    ...
    async def send_message(
        self, conversation_data: Dict[str, Any], new_message: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Use direct LLM service for real AI responses, injecting a
            # lightweight patient snapshot (dev convenience) to enhance context.
            self.logger.info("Using direct LLM service for real AI responses")
            try:
                enhanced_system = self.prompts_service.build_enhanced_system_prompt(
                    base_id="system_prompt",
                    patient_snapshot=get_any_patient_snapshot(),
                )
                ...
                if direct_response and direct_response.get("content"):
                    ...
                    self.logger.info(
                        f"Direct AI response: {direct_response.get('model_used')} -> {response[:100]}..."
                    )
                else:
                ...
                self.logger.warning(
                    f"Direct AI service failed: {e} - falling back to mock LLM"
                )
    ...
    def get_active_conversations(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            conv_id: {
                ...
                "last_message": (
                    conv.messages[-1].timestamp.isoformat() if conv.messages else None
                ),
            }
    ...
    def create_conversation(self, user_id: str) -> str:
        ...
        conversation = ConversationHistory(
            messages=[], conversation_id=conversation_id, created_at=datetime.now()
        )
