...
class NHSTerminologyService:
    ...
    async def _make_request(self, endpoint: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if e.response.status_code == 401:
                logger.warning("Authentication failed - may need valid credentials for this terminology")
    ...
    def _parse_lookup_response(self, response: Dict[str, Any], system: str) -> Optional[TerminologyConcept]:
            ...
            if name == "name":
                ...
                for part in prop_params:
                    if part.get("name") == "code":
                        ...
                        prop_value = part.get("valueString") or part.get("valueCoding") or part.get("valueBoolean")
                if prop_code:
    ...
    async def search_concepts(
        ...
        try:
            ...
            logger.error(f"Concept search failed for {system_uri} with filter '{filter_text}': {e}")
            return []
    
    def _parse_expand_response(self, response: Dict[str, Any], system: str) -> List[TerminologyConcept]:
    ...
    async def translate_code(
        ...
        """
        source_uri = source_system.value if isinstance(source_system, TerminologySystem) else source_system
        target_uri = target_system.value if isinstance(target_system, TerminologySystem) else target_system
        ...
        try:
            ...
            mappings = self._parse_translate_response(result, source_uri, source_code, target_uri)
            self.cache[cache_key] = mappings
            return mappings
            
        except Exception as e:
            logger.error(f"Code translation failed from {source_uri}#{source_code} to {target_uri}: {e}")
            return []
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "healthy",
                "server_name": result.get("software", {}).get("name", "NHS Terminology Server"),
...
class ClinicalCodingService:
    ...
    async def code_diagnosis(self, diagnosis_text: str) -> List[Dict[str, Any]]:
        ...
        for concept in concepts:
            relevance_score = self._calculate_text_relevance(diagnosis_text, concept.display)
