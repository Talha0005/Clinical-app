...
class PromptsService:
    ...
    def load_prompts(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Invalid JSON in prompts file {self.prompts_file}: {e}")
            return self._create_default_prompts()
        except PermissionError as e:
            logger.error(f"Permission denied reading prompts file {self.prompts_file}: {e}")
            return self._create_default_prompts()
    ...
    def save_prompts(self) -> bool:
        ...
        try:
            ...
            self.prompts_cache["metadata"]["total_prompts"] = len(self.prompts_cache.get("prompts", {}))
    ...
    def update_prompt(self, prompt_id: str, updates: Dict[str, Any], user_id: str = None) -> bool:
        ...
        try:
            ...
            original_prompt = prompts[prompt_id].copy()
            
            # Update the prompt
            prompt = prompts[prompt_id]
            
            # Update allowed fields
            allowed_fields = ["name", "description", "content", "category", "is_active"]
            changed_fields = {}
            for field, value in updates.items():
                if field in allowed_fields and prompt.get(field) != value:
                    changed_fields[field] = {"old": prompt.get(field), "new": value}
                    prompt[field] = value
            ...
            if changed_fields:
                logger.info(f"AUDIT: Prompt {prompt_id} updated by user {user_id or 'unknown'} - "
                          f"Version {old_version} -> {prompt['version']} - "
                          f"Changed fields: {list(changed_fields.keys())}")
                
                # Log content changes for medical prompts specifically
                if prompt.get("category") in ["medical", "system"] and "content" in changed_fields:
                    logger.warning(f"MEDICAL_AUDIT: Critical prompt content changed - "
                                 f"Prompt: {prompt_id}, User: {user_id or 'unknown'}, "
    ...
    def create_prompt(self, prompt_data: Dict[str, Any]) -> bool:
        ...
        Args:
            prompt_data: Prompt data including id, name, description, content, etc.
        ...
        default_prompts = {
            "prompts": {
                "system_prompt": {
                    "id": "system_prompt",
                    "name": "System Prompt",
                    "description": "Main system prompt used for conversation initialization",
                    "category": "system",
                    "content": "You are Dr. Hervix, a digital GP consultant for the NHS DigiClinic platform.",
                    "version": 1,
                    "created_at": datetime.now().isoformat(),
                    "updated_at": datetime.now().isoformat(),
                    "is_active": True
