...
class ClinicalCodesCache:
    ...
    def _initialize_symptom_mappings(self) -> Dict[str, SymptomMapping]:
        ...
        mappings["productive_cough"] = SymptomMapping(
            ...
            differential_diagnosis=["Bronchitis", "Pneumonia", "COPD exacerbation"],
        )
        ...
        mappings["pleuritic_chest_pain"] = SymptomMapping(
            ...
            differential_diagnosis=["Pleuritis", "Pneumonia", "Pulmonary embolism"],
        )
    ...
    def find_codes_for_symptom(self, symptom_text: str) -> Optional[SymptomMapping]:
        ...
        for _key, mapping in self.symptom_mappings.items():
            for keyword in mapping.keywords:
                ...
                if kw_raw in s_raw or kw_norm in s_norm or kw_compact in s_compact:
                    return mapping
        ...
        if (
            ("chest pain" in s_norm)
            or ("chestpain" in s_compact)
            or ("chest ache" in s_norm)
        ):
        ...
        for code in self.codes_db.values():
            if code.category == "condition":
                ...
                if disp and (
                    disp.lower() in s_raw
                    or disp_norm in s_norm
                    or disp_compact in s_compact
                ):
                ...
                for syn in code.synonyms:
                    ...
                    if syn and (
                        syn.lower() in s_raw
                        or syn_norm in s_norm
                        or syn_compact in s_compact
                    ):
                        return SymptomMapping(
    ...
    def generate_medical_report(
        self, symptoms: List[str], patient_info: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        for symptom in symptoms:
            ...
            if mapping:
                ...
                report["clinical_codes"].extend(
                    [code.to_dict() for code in mapping.primary_codes]
                )
        ...
        if report["symptoms_analyzed"]:
            ...
            if any("chest pain" in s for s in symptom_texts):
                report["recommendations"].append(
                    "Consider ECG if cardiac symptoms present"
                )
...
def search_codes_by_keyword(keyword: str) -> List[ClinicalCode]:
    ...
    for code in clinical_codes_cache.codes_db.values():
        if (
            keyword_lower in code.display.lower()
            or keyword_lower in (code.description or "").lower()
            or any(keyword_lower in synonym.lower() for synonym in code.synonyms)
        ):
            results.append(code)
...
if __name__ == "__main__":
    # Test the system
    test_symptoms = ["cough", "chest pain", "fever", "shortness of breath"]
