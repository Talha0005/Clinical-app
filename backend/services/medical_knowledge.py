...
class MedicalKnowledgeBase:
    ...
    def __init__(
        ...
        self.clinical_coding = ClinicalCodingService(nhs_terminology) if nhs_terminology else None
        ...
        hypertension_guideline = MedicalGuideline(
            ...
            recommendations=[
                ClinicalRecommendation(
                    ...
                    contraindications=["Atrial fibrillation with frequent irregular heartbeats"],
                    monitoring_requirements=["Annual blood pressure check", "Cardiovascular risk assessment"]
                ),
                ClinicalRecommendation(
                    ...
                    monitoring_requirements=["Blood pressure monitoring", "Renal function", "Electrolytes"]
                )
            ],
            quality_indicators=[
        ...
        diabetes_guideline = MedicalGuideline(
            ...
            recommendations=[
                ClinicalRecommendation(
                    ...
                    monitoring_requirements=["HbA1c every 3-6 months", "Annual renal function", "Vitamin B12 levels"]
                )
            ],
            quality_indicators=[
    ...
    async def search_evidence(
        ...
        if condition:
            condition_result = await self.nice_data_source.get_condition_by_name(condition)
            if condition_result:
                evidence_list.extend(self._extract_evidence_from_condition(condition_result))
        ...
        for guideline in self.guidelines_cache.values():
            if query.lower() in guideline.title.lower() or query.lower() in guideline.scope.lower():
        ...
        sorted_evidence = sorted(unique_evidence, key=lambda e: e.quality_score or 0, reverse=True)
        
        # Cache results
        self.evidence_cache[cache_key] = sorted_evidence[:limit]
        return sorted_evidence[:limit]
    
    def _extract_evidence_from_condition(self, condition: MedicalCondition) -> List[ClinicalEvidence]:
        ...
        if condition.source_url:
            evidence = ClinicalEvidence(
                ...
                publication_date=datetime.fromisoformat(condition.last_updated) if condition.last_updated else None,
    ...
    def _deduplicate_evidence(self, evidence_list: List[ClinicalEvidence]) -> List[ClinicalEvidence]:
    ...
    async def get_clinical_recommendations(
        ...
        for guideline_key, guideline in self.guidelines_cache.items():
            if condition_lower in guideline_key or condition_lower in guideline.title.lower():
        ...
        if nice_condition:
            nice_recommendations = self._extract_recommendations_from_condition(nice_condition)
    ...
    def _extract_recommendations_from_condition(self, condition: MedicalCondition) -> List[ClinicalRecommendation]:
    ...
    async def assess_drug_interactions(
        ...
        for current_med in medications:
            ...
            for pattern_med, pattern_data in interaction_patterns.items():
                if (pattern_med in new_med_lower and 
                    any(interacting_med in current_med_lower 
                ...
                elif (pattern_med in current_med_lower and 
                      any(interacting_med in new_med_lower 
                          for interacting_med in pattern_data["interacts_with"])):
        ...
        if high_severity_count > 0:
            warnings.append("High-severity drug interactions detected - clinical review required")
        
        moderate_severity_count = sum(1 for i in interactions if i["severity"] == "moderate")
        if moderate_severity_count > 0:
            warnings.append("Moderate drug interactions detected - monitoring recommended")
        ...
        return {
            ...
            "warnings": warnings,
            "recommendation": "Consult pharmacist for comprehensive interaction review" if interactions else "No major interactions detected",
...
class EvidenceBasedResponseGenerator:
    ...
    async def generate_evidence_based_response(
        ...
        for condition in potential_conditions:
            ...
            condition_recommendations = await self.knowledge_base.get_clinical_recommendations(
    ...
    async def _extract_conditions_from_query(self, query: str) -> List[str]:
        ...
        if medical_terms:
            search_result = await self.knowledge_base.nice_data_source.search_conditions(
    ...
    async def _generate_ai_response_with_evidence(
        ...
        if recommendations:
            ...
            for i, rec in enumerate(recommendations[:3], 1):
                recommendations_summary += f"{i}. {rec.recommendation} (Strength: {rec.strength})\n"
...
"""
    ...
    def _calculate_evidence_quality(self, evidence_list: List[ClinicalEvidence]) -> float:
