...
def verify_key_setup():
    ...
    print("📁 File Check:")
    print(
        f"Private key: {'✅' if private_key_path.exists() else '❌'} {private_key_path}"
    )
    print(
        f"Public key:  {'✅' if public_key_path.exists() else '❌'} {public_key_path}"
    )
    print(
        f"Public JSON: {'✅' if public_json_path.exists() else '❌'} {public_json_path}"
    )
    print()

    if not all(
        [private_key_path.exists(), public_key_path.exists(), public_json_path.exists()]
    ):
    ...
    try:
        ...
        # Create JWT
        token = jwt.encode(
            payload, private_key, algorithm="RS512", headers={"kid": "doogie-ai-2024"}
        )
