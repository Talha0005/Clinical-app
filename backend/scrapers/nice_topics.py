...
def clean_extracted_content(raw_text: str) -> str:
    ...
    filtered_lines = []
    skip_phrases = [
        'copyright', 'licence', 'license', 'cookies', 'navigation', 'menu',
        'skip to', 'back to top', 'accessibility', 'print this page', 'feedback',
        'find out more', 'browse topics', 'search for topics', 'nice cks',
        'health topics a to z', 'summary', 'management', 'background information',
        'scenario', 'supporting evidence', 'references', 'how this topic',
        'goals and outcome', 'have i got the right', 'up-to-date',
        'clarity informatics', 'agilio software', 'end user licence',
        '"@', 'itemlistelement', 'breadcrumblist', 'listitem'
    ]
    ...
    for line in lines:
        ...
        if (len(line) < 15 or 
            any(skip in line_lower for skip in skip_phrases) or
            line.startswith('{') or line.startswith('[') or
            '"@' in line or 'http://' in line or 'https://' in line):
            continue
        
        # Keep lines that seem to contain medical content
        medical_indicators = [
            'aaa', 'aneurysm', 'screening', 'diagnosis', 'treatment', 'patient', 
            'medical', 'condition', 'symptom', 'risk', 'factor', 'prevalence',
            'management', 'therapy', 'clinical', 'health', 'nhs', 'care',
            'ultrasound', 'scan', 'vascular', 'surgeon', 'surveillance'
        ]
        
        # If line has medical content or is substantial, keep it
        if (any(indicator in line_lower for indicator in medical_indicators) or
            len(line) > 50):
...
async def extract_section_content(page, section_name: str) -> str:
    ...
    try:
        ...
        if chapter_count > 0:
            ...
            if chapter_count > 1:
                print(f"    Found {chapter_count} ChapterBody sections, combining all...")
                all_text = []
        ...
        if section_name.lower() == 'background':
            ...
            subsection_patterns = [
                'Definition', 'Prevalence', 'Risk factors', 'Eligibility for routine AAA screening',
                'Programme coordination and management', 'The AAA screening test', 
                        ...
                        if await heading.count() > 0:
                            ...
                            for content_selector in following_content_selectors:
                                ...
                                if await content_elem.count() > 0:
                                    section_text = await content_elem.first.text_content()
                                    if section_text and len(section_text.strip()) > 20:
                                        subsection_content[pattern] = section_text.strip()[:500]
                                        print(f"      ✅ Found {pattern}: {len(section_text)} chars")
                                        break
                            break
            ...
            if subsection_content:
                ...
                for subsection_name, subsection_text in subsection_content.items():
                    formatted_subsections.append(f"{subsection_name}:\n{subsection_text}")
...
async def extract_topic_details(page, url: str) -> Dict[str, str]:
    ...
    try:
        ...
        if ("licence" in page_text.lower() or "license" in page_text.lower() or 
            "copyright" in page_text.lower() or "terms" in page_text.lower()):
            print("⚠️  Detected license/terms page, looking for continue/accept buttons...")
            ...
            if ("licence" in new_page_text.lower() and 
                len(new_page_text) < 1000):  # Still seems like license page
        ...
        for section in content_sections:
            ...
            if count > 0:
                ...
                for i in range(count):
                    ...
                    if href and 'background-information' not in href:  # Skip background info subitems
                        section_links[section] = {'href': href, 'text': text, 'element': link}
                        print(f"    - {text}: {href}")
                        break
            ...
            for i, selector in enumerate(alternative_selectors):
                ...
                if alt_count > 0:
                    ...
                    if alt_text and len(alt_text.strip()) > 100:
                        ...
                        if summary_content:
                            print(f"  ✅ Found clean content with alternative selector: {selector}")
                            break
        ...
        if summary_content:
            ...
            for i, line in enumerate(summary_lines):
                if any(word in line.lower() for word in ['screening', 'diagnosis', 'test', 'ultrasound']):
                    diag_section = '\n'.join(summary_lines[i:i+5])
                    if not details["diagnosis"] or len(diag_section) > len(details["diagnosis"]):
                        details["diagnosis"] = diag_section[:800]
            
            # Look for treatment/management info
            for i, line in enumerate(summary_lines):
                if any(word in line.lower() for word in ['treatment', 'management', 'primary care', 'role']):
                    treatment_section = '\n'.join(summary_lines[i:i+8])
                    if not details["treatments"] or len(treatment_section) > len(details["treatments"]):
        ...
        target_sections = [
            ...
            {'name': 'Scenario', 'keywords': ['scenario', 'aaa screening'], 'key': 'scenario'},
                ...
                try:
                    ...
                    if count > 0:
                        for i in range(count):
                            ...
                            if text and any(keyword in text.lower() for keyword in section_info['keywords']):
                                print(f"  📝 Found {section_name} link: {text} -> {href}")
                                ...
                                try:
                                    ...
                                    section_content = await extract_section_content(page, section_name)
                                    if section_content:
                                        additional_sections[section_key] = section_content
                                        print(f"  ✅ Extracted {len(section_content)} characters from {section_name}")
                                    else:
                                        print(f"  ❌ No content extracted from {section_name}")
        ...
        for i, selector in enumerate(symptoms_selectors):
            print(f"  📝 Trying symptoms selector {i+1}/{len(symptoms_selectors)}: {selector}")
            ...
            if count > 0:
                ...
                print(f"    Text preview: {symptoms_text[:100] if symptoms_text else 'None'}...")
        ...
        if not any(details.values()):
            ...
            for i in range(min(5, count)):
                ...
                if text and len(text.strip()) > 30:
                    ...
                    if not any(skip_word in text_lower for skip_word in 
                              ['navigation', 'menu', 'home', 'search', 'contact', 'about us', 
                               'follow us', 'sign up', 'newsletter', 'cookies', 'privacy']):
            ...
            if not details["summary"]:
                ...
                content_links = page.locator('a:has-text("Background"), a:has-text("Management"), a:has-text("Scenario")')
                if await content_links.count() > 0:
                    try:
                        ...
                        retry_paragraphs = page.locator('main p:not(:has-text("copyright")):not(:has-text("licence"))')
...
async def scrape_nice_topics() -> Dict[str, Dict[str, str]]:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            try:
                accept_cookies = page.locator('button:has-text("Accept all cookies"), button:has-text("Accept"), button[aria-label*="Accept"]')
            ...
            try:
                eula_accept = page.locator('button:has-text("Accept"), button:has-text("I agree"), button:has-text("Continue")')
                ...
                if count > 0:
                    for i in range(count):
                        ...
                        if title and href and title.strip():
                            title = title.strip()
                            full_url = href if href.startswith('http') else f"https://cks.nice.org.uk{href}"
...
async def save_detailed_topics(topics: Dict[str, Dict[str, str]], file_path: str = None) -> str:
...
async def scrape_and_save_detailed_topics(limit: int = None) -> str:
    ...
    print(f"🎉 Detailed scraping complete! {len(detailed_topics)} topics saved to {file_path}")
    return file_path
