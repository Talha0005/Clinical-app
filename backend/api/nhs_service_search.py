    ...
    async def _get_headers(self) -> Dict[str, str]:
        ...
        if self.oauth_client:
            try:
                ...
                logger.warning(f"OAuth authentication failed, falling back to API key: {e}")
        ...
        try:
            async with self.session.get(url, headers=headers, params=params) as response:
                if response.status == 200:
                    ...
                    logger.error(f"Service search failed: {response.status} - {error_text}")
        ...
        try:
            async with self.session.get(url, headers=headers, params=params) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"Organisation search failed: {response.status} - {error_text}")
