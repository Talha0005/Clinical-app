...
async def test_voice_text_websocket(
            ...
            try:
                ...
                if message.get("type") == "auth":
                    try:
                        ...
                        payload = jwt.decode(
                            message["token"], SECRET_KEY, algorithms=[ALGORITHM]
                        )
                ...
                # Handle text transcript
                    ...
                    if not authenticated:
                        await websocket.send_json(
                            {"type": "error", "error": "Must authenticate first"}
                        )
                        continue
                    ...
                    if is_final and transcript_text and llm_router:
                        try:
                            logger.info(
                                f"Generating LLM response for: {transcript_text}"
                            )
                ...
                await websocket.send_json(
                    {"type": "error", "error": f"Processing error: {str(e)}"}
                )
