...
JWT_SECRET = (
    (getattr(_auth, "SECRET_KEY", None) if "_auth" in globals() and _auth else None)
    or os.getenv("JWT_SECRET")
    or os.getenv("SECRET_KEY")
    or ("dev-secret-change-me")
)
JWT_ALGORITHM = (
    (getattr(_auth, "ALGORITHM", None) if "_auth" in globals() and _auth else None)
...
AGENTS_ENABLED = os.getenv("AGENTS_ENABLED", "false").strip().lower() in {
    "1",
    "true",
    "yes",
}
...
async def voice_stream_endpoint(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        await websocket.send_json(
            {
                ...
                "message": (
                    "Voice processing WebSocket connected. " "Please authenticate."
                ),
            }
        )
        ...
        async def audio_generator():
            ...
            while True:
                try:
                            ...
                            await websocket.send_json(
                                {
                                    "type": "error",
                                    "error": (
                                        "Must authenticate before sending" " audio"
                                    ),
                                }
                            )
                            continue
                                ...
                                await websocket.send_json(
                                    {
                                        "type": "error",
                                        "error": (
                                            "Audio message missing 'data'" " field"
                                        ),
                                    }
                                )
                                continue
                                ...
                                await websocket.send_json(
                                    {
                                        ...
                                        "message": (
                                            "Audio stream received." " Transcribing…"
                                        ),
                                    }
                                )
                            yield audio_data
                    ...
                    await websocket.send_json(
                        {"type": "error", "error": f"Audio reception error: {str(e)}"}
                    )
                    break
        ...
        try:
            await websocket.send_json(
                {"type": "error", "error": f"WebSocket error: {str(e)}"}
            )
        except Exception:
            pass
...
async def upload_voice_file(
    ...
    user_id = payload.get("sub", "authenticated_user")
    current_session_id = (
        session_id or f"upload_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    try:
        ...
        transcription_result = await voice_service.transcribe_audio_file(
            file_path, current_session_id, user_id
        )
...
async def voice_health_check():
    ...
    try:
        ...
        return {"success": False, "error": str(e), "voice_service": {"status": "error"}}
