...
class NHSOAuthClient:
        ...
        try:
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    logger.info(f"Successfully obtained NHS API token, expires in {token.expires_in}s")
                    return token
    ...
    def _create_client_assertion(self) -> str:
        ...
        payload = {
            ...
            "exp": int((now + timedelta(minutes=5)).timestamp()),  # Expiration (5 minutes from now)
            "iat": int(now.timestamp()),  # Issued at (exactly now)
        }
        # Don't include nbf (not before) as it might cause issues
        
        logger.info(f"Creating JWT assertion with exp={payload['exp']}, iat={payload['iat']}")
    ...
    async def get_authenticated_headers(self, additional_headers: Optional[Dict[str, str]] = None) -> Dict[str, str]:
        """
    ...
    async def test_connection(self) -> tuple[bool, str]:
        ...
        try:
            ...
            return (True, f"Successfully authenticated with NHS API. Token expires at {token.expires_at}")
