...
async def analyze_symptoms(
    request: SymptomRequest, current_user: str = Depends(verify_token)
):
    """
    ...
    if not clinical_codes_cache:
        raise HTTPException(
            status_code=503, detail="Clinical codes service not available"
        )
    ...
    try:
        ...
        logger.info(
            f"Generated medical report with {len(report['clinical_codes'])} codes"
        )

        return MedicalReportResponse(success=True, **report)
...
async def search_clinical_codes(
    ...
    if not search_codes_by_keyword:
        raise HTTPException(
            status_code=503, detail="Clinical codes search service not available"
        )
...
async def get_common_symptoms(current_user: str = Depends(verify_token)):
    """
    ...
    try:
        ...
        for key, mapping in clinical_codes_cache.symptom_mappings.items():
            symptom_info = {
                ...
                "body_systems": list(
                    set(
                        code.body_system
                        for code in mapping.primary_codes
                        if code.body_system
                    )
                ),
            }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get common symptoms: {str(e)}"
        )


@router.post("/quick-code")
async def quick_symptom_code(
    request: QuickCodeRequest, current_user: str = Depends(verify_token)
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to code symptom: {str(e)}")
...
async def generate_example_report(current_user: str = Depends(verify_token)):
    ...
    example_patient = {"age": 35, "gender": "male", "duration": "5 days"}
