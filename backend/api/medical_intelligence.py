...
class HistoryTakingRequest(BaseModel):
    patient_message: str = Field(
        ..., description="Patient's description of their problem"
    )
...
class SymptomTriageRequest(BaseModel):
    ...
    associated_symptoms: List[str] = Field(
        default=[], description="Associated symptoms"
    )
...
class TerminologyLookupRequest(BaseModel):
    ...
    properties: Optional[List[str]] = Field(
        None, description="Additional properties to retrieve"
    )
...
class ClinicalCodingRequest(BaseModel):
    diagnosis_text: str = Field(
        ..., description="Natural language diagnosis description"
    )
...
class EvidenceBasedResponseRequest(BaseModel):
    ...
    clinical_context: Optional[Dict[str, Any]] = Field(
        None, description="Clinical context"
    )
    patient_factors: Optional[Dict[str, Any]] = Field(
        None, description="Patient factors"
    )
...
async def lookup_terminology_concept(
    ...
    try:
        ...
        if not system:
            raise HTTPException(
                status_code=400,
                detail=f"Unsupported terminology system: {request.system}",
            )

        async with nhs_terminology:
            concept = await nhs_terminology.lookup_concept(
                system=system, code=request.code, properties=request.properties
            )
    ...
    try:
        ...
        async with nhs_terminology:
            concepts = await nhs_terminology.search_concepts(
                system=system, filter_text=request.filter_text, limit=request.limit
            )
...
async def translate_code(
    ...
    try:
        ...
        if not source_system or not target_system:
            raise HTTPException(
                status_code=400, detail="Unsupported terminology system"
            )
...
async def code_clinical_diagnosis(
    ...
    try:
        ...
        async with nhs_terminology:
            coded_diagnoses = await clinical_coding.code_diagnosis(
...
async def analyze_medical_image(
    ...
    analysis_level: str = Form(
        "clinical", description="Analysis level: basic, clinical, diagnostic, detailed"
    ),
...
async def search_clinical_evidence(
    ...
    token_data: dict = Depends(verify_token),
    evidence_generator: EvidenceBasedResponseGenerator = Depends(
    ...
    try:
        evidence_list = await evidence_generator.knowledge_base.search_evidence(
            query=request.query, condition=request.condition, limit=request.limit
        )
...
async def assess_drug_interactions(
    ...
    try:
        interaction_assessment = (
            await evidence_generator.knowledge_base.assess_drug_interactions(
                medications=request.medications, new_medication=request.new_medication
            )
        )
...
async def get_compliance_report(
    ...
    try:
        if not medical_observability or not medical_observability.enabled:
            raise HTTPException(
                status_code=503, detail="Observability service not available"
            )
