...
async def search_snomed(query: str, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        return [{"type": "text", "text": f"Error searching SNOMED CT: {str(e)}"}]
...
async def search_medications(name: str, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    try:
        async with NHSTerminologyServer() as server:
            ...
            if not medications:
                return [{"type": "text", "text": f"No medications found for '{name}'"}]
        ...
        return [{"type": "text", "text": f"Error searching medications: {str(e)}"}]
...
async def validate_code(code: str, system: str) -> List[Dict[str, Any]]:
    ...
    try:
        async with NHSTerminologyServer() as server:
            ...
            if not system_url:
                return [{"type": "text", "text": f"Unknown system: {system}"}]
        ...
        return [{"type": "text", "text": f"Error validating code: {str(e)}"}]
