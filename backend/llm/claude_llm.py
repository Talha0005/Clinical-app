...
class ClaudeLLM(BaseLLM):
    ...
    async def generate_response(
        ...
        **kwargs
        ...
        try:
            ...
            logger.error(f"Error calling Claude API with key {self._mask_api_key(self.api_key)}: {str(e)}")
    ...
    def create_medical_system_prompt(self) -> str:
        ...
        try:
            ...
            logger.warning(f"Could not import prompts_service, using fallback prompt: {e}")
        except Exception as e:
    ...
    async def generate_streaming_response(
        ...
        **kwargs
        ...
        try:
            ...
            if system_prompt is None:
                system_prompt = self.create_medical_system_prompt()
                logger.info(f"🎯 Using default medical system prompt ({len(system_prompt)} chars)")
            
            # Add the new message to conversation
            conversation.add_message("user", new_message)
            logger.info(f"🎯 Added user message to conversation (total: {len(conversation.messages)} messages)")
            ...
            logger.info(f"🎯 Model: {self.model_name}, max_tokens: {max_tokens}, temp: {temperature}")
                ...
                async with client.stream(
                    ...
                    if response.status_code != 200:
                        ...
                        try:
                            ...
                            logger.error(f"❌ Claude API error details: {error_content}")
                        except:
                            pass
                        ...
                        if line.startswith('data: '):
                            ...
                            try:
                                ...
                                if chunk.get('type') == 'content_block_delta':
                                    ...
                                    logger.info(f"🎯 Other chunk type: {chunk.get('type')}")
                                    
                            except json.JSONDecodeError as e:
                                logger.warning(f"⚠️  JSON decode error on line: {data[:100]}... Error: {e}")
                                continue
