...
class MockPatientDB:
    ...
    def find_patient(self, patient_name: str, national_insurance: str) -> Optional[Patient]:
        """Find a patient by name and national insurance number."""
        patients = self.load_patients()
        
        for patient in patients:
            if (patient.name.lower() == patient_name.lower() and 
                patient.national_insurance == national_insurance):
                return patient
    ...
    def get_patient_list(self) -> List[Dict[str, str]]:
        ...
        return [
            {
                "name": patient.name,
                "national_insurance": patient.national_insurance
            }
            for patient in patients
        ]
    ...
    def create_new_patient(self, patient_data: Dict[str, Any]) -> bool:
        ...
        for patient in existing_patients:
            if patient.national_insurance == new_patient.national_insurance:
                raise ValueError(f"Patient with National Insurance {new_patient.national_insurance} already exists")
